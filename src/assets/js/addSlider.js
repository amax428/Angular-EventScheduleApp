  ...
  Object.defineProperty(this, "value", {
    ...
    set: function(newVal){
      ...
      if(!this._settings.range){
        ...
        if($('.time-now').hasClass('firsttime')){
          $('.time-now').show();
          $('.time-now').removeClass('firsttime');
          var today = new Date();
          currentTime = today.getHours()+'.'+today.getMinutes();
          currentTime1 = today.getHours()+':'+today.getMinutes();
          l=currentTime;
          $('.start-range').text(toFunc(self._settings.formatter).call(self, l));
          // $('.start-range').hide();
        }else{
          $('.time-now').hide();
          $('.start-range').show();
        ...
        this._elements.each(function(i,el){
          ...
          $('.end-range').text(toFunc(self._settings.formatter).call(self, h));
